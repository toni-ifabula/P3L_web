<template>
    <v-main class="list">
        <h3 class="text-h3 font-weight-medium mb-5"> Laporan Pengeluaran </h3>
        ...
        <v-dialog v-model="dialog" max-width="650px">
            <v-card>
                <v-card-text ref="document">
                    ...
                    <h6 v-if="currentTipe == 'Bulanan'" style="text-align: center; color: black">Laporan Pengeluaran Bulanan</h6>
                    <h6 v-if="currentTipe == 'Tahunan'" style="text-align: center; color: black">Laporan Pengeluaran Tahunan</h6>
                    ...
                    <br>
                    <h5 style="color: black">Total Pengeluaran Tahun {{ tahun }}</h5>
                    <v-data-table
                        v-if="currentTipe == 'Bulanan'"
                        :hide-default-footer="true"
                        :headers="headersBulanan"
                        :items="totalPengeluaranBulanan"
                    >
                    </v-data-table>
                    <v-data-table
                        v-if="currentTipe == 'Tahunan'"
    ...
    methods: {
        showLaporan() {
            ...
            if (this.tipe == "Bulanan") {
                ...
                this.getPengeluaranBulanan(this.tahun);
                this.dialog = true;
            } else if(this.tipe == "Tahunan") {
                this.currentTipe = "Tahunan"
                this.getPengeluaranTahunan(this.tahunAwal, this.tahunAkhir)
                this.dialog = true;
            }
        },
        getPengeluaranBulanan(tahun) {
            var url = this.$api + "/laporanPengeluaranBulanan/" + tahun;
            this.$http
                .get(url, {
                    headers: {
                        Authorization:
                ...
                .then((response) => {
                    ...
                    this.totalPengeluaranBulanan = response.data.total;
                })
                .catch((error) => {
        ...
        getPengeluaranTahunan(tahunAwal, tahunAkhir) {
            var url = this.$api + "/laporanPengeluaranTahunan/" + tahunAwal + "/" + tahunAkhir;
            this.$http
                .get(url, {
                    headers: {
                        Authorization:
                ...
                .then((response) => {
                    ...
                    this.sideDish = response.data.sideDish;
                    this.minuman = response.data.minuman;
                    this.totalPengeluaranTahunan = response.data.total;
                })
                .catch((error) => {
                    this.error_message = error.response.data.message;
                    this.color = "red";
